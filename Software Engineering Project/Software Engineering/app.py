...
def registercollege():
    ...
    if request.method == 'POST' and form.validate():
        ...
        return redirect(url_for('LoginCollege'))
...
class ReportForm(Form):
    ...
    Complain = TextAreaField('Complain')
...
def updatebio(College_Name):
    ...
    # Get article by id
    result = cur.execute("SELECT * FROM College_Details WHERE College_Name = %s", [College_Name])
    ...
    if request.method == 'POST' and form.validate():
        ...
        # Execute
        cur.executemany(stmt,(Fees, Courses, Application_Link, Contact, Nirf_Rank, Average_CTC, Notable_Companies, JEE_MAINS_Rank_Criteria, About, Website_Link, [College_Name]))
        # Commit to DB
        mysql.connection.commit()
